...
// followed by a seven-digit local number. The first three digits of the local number represent the exchange code, followed by the unique four-digit number which is the subscriber number.
...
def number = "1613-995-0253"
def phone = []

for(x in number) {
    if(x.isNumber()) phone << x
}
